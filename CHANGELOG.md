...
This release addresses **8 CRITICAL and HIGH severity security vulnerabilities** discovered through comprehensive code review:

#### Deserialization Security (CRITICAL)
- **Fixed #220**: Hardened deserialization filter against RCE gadget chains
  - Added SerializedLambda and AnnotationInvocationHandler to blacklist
  - Replaced overly broad `org.flossware.*` wildcard with explicit class enumeration
  - 91% coverage of common deserialization attack vectors
  
- **Fixed #216**: Prevented array deserialization bypass (RCE vulnerability)
  - Added array component type extraction and validation
  - Expanded DANGEROUS_CLASSES blacklist (11 → 19 classes)
  - Now blocks `HashMap[]`, `PriorityQueue[]`, and other gadget chain arrays
  - Added 8 comprehensive security tests

#### File Security (HIGH)  
- **Fixed #221**: Eliminated TOCTOU race conditions in file operations
  - Replaced `Files.newInputStream()` with `FileChannel.open()` using `NOFOLLOW_LINKS`
  - Added hardlink identity tracking via `fileKey()` comparison
  - Added 5 multi-threaded race condition tests
  
- **Fixed #222**: Performance optimization with maintained security
  - Added `validatePathTraversalWithRealBase()` for batch operations
  - Defensive parameter validation prevents bypass attacks
  - Added 8 security tests for parameter injection

- **Fixed #217**: Added path traversal validation to deprecated `getFileInputStream()`
  - Now validates against `../` patterns before file access
  - Added JavaDoc security warnings (CWE-22, CWE-367)
  - Added 4 security tests

- **Fixed #215**: Added path traversal validation to deprecated `ensureFileExists()`
  - Validates paths before file existence check
  - Added JavaDoc security warnings
  - Added 3 security tests

- **Fixed #219**: Path validation now supports non-existent files
  - Uses `normalize() + toAbsolutePath()` for non-existent paths
  - Falls back to `toRealPath()` for existing files (symlink resolution)
  - Added 6 comprehensive tests

- **Fixed #218**: Added path traversal protection to PropertyUtil.fromFile()
  - Validates against parent directory escape attempts
  - Rejects `../` patterns and multi-level traversal
  - Added JavaDoc security warnings

### Changed
- Expanded `DANGEROUS_CLASSES` blacklist from 11 to 19 classes
- All deprecated File-based methods now include security validation
- Improved JavaDoc documentation with `@apiNote` security warnings

### Tests
- Added 40+ new security tests across all affected components
- All 321 tests passing (100% success rate)
- Comprehensive adversarial testing performed

### Migration Notes
- **BREAKING**: Code relying on deserialization of `HashMap`, `PriorityQueue`, `TreeSet`, etc. will now fail
- **BREAKING**: Path traversal attempts in deprecated methods will now throw `FileException`
- Recommended: Migrate away from deprecated methods to NIO.2 Path-based APIs
